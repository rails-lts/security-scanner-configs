...
def define_generate_rake_tasks_for(task_name, file_name, generator_class)
  all_lts_dirs = LTS_VERSIONS.map { |version| "configs/rails_lts/#{version.gsub('.', '_')}" }
  all_rack_dirs = RACK_VERSIONS.map { |version| "configs/rack/#{version.gsub('.', '_')}" }
  ...
  LTS_VERSIONS.each do |lts_version|
  ...
  RACK_VERSIONS.each do |lts_version|
...
# API tasks
desc 'Generate missing artifacts'
